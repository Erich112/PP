...
fun main() {
    /*
    val file = "/home/e/IdeaProjects/untitled/src/main/resources/ebook.txt"
    val myList = mutableListOf<String>()
    File(file).useLines { lines -> myList.addAll(lines) }
    val whespaceRegex = Regex("\\s\\s+")
    for (lines in myList) {
            println(whespaceRegex.replace(lines, ""))
    }
    */
...
// Find all matches
    for(regex in listOf(ipRegex, emailRegex, timeRegex)) {
